...
export default function ClosedLoopStepper({ onStepSelect }) {
  ...
  const steps = [
    ...
    { id: "gaps", title: "3. Traffic Gaps", desc: "M4 Headway Mining", icon: "schedule", tab: "time_space_canvas" },
    { id: "grouping", title: "4. Joint Grouping", desc: "Civil + S&T + TRD", icon: "layers", tab: "joint_shadow_matrix" },
    ...
    { id: "bdms", title: "9. BDMS Dispatch", desc: "Idempotent Signed Payload", icon: "send", tab: "bdms" },
    { id: "replan", title: "10. Live Replan", desc: "Disruption Execution", icon: "sync", tab: "replan" }
  ];

  const handleStepClick = (idx, tab) => {
    setActiveStep(idx);
    if (onStepSelect) onStepSelect(tab, steps[idx]);
  };
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div
              ...
              className={`p-2 rounded-lg cursor-pointer transition-all flex flex-col items-center text-center border ${isActive ? "bg-secondary text-on-secondary border-secondary-fixed shadow-md scale-105 font-bold" : "bg-primary-container/40 text-on-primary-container border-transparent hover:bg-primary-container hover:text-on-primary"}`}
            >
